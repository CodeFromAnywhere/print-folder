...
*/
  ...
  const pdfPages = await mapMany(
    ...
    async (item, index) => {
      ...
      await page.pdf({
        ...
        scale: 1.5,
        footerTemplate:
          '<div style="font-size:20px; margin-left:20px;"><span class="title"></span> - <span class="pageNumber"></span> of <span class="totalPages"></span></div>',
      });
  ...
  await page.setContent(
    `<html><body><pre>${tableOfContents}</pre></body></html>`,
  );
